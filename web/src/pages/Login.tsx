...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        await axios.post(`${API_BASE}/register`, { email, password, full_name: name });
        setIsRegister(false);
        setErr('Account created. Log in to continue.');
      } else {
      ...
      setErr(error?.response?.data?.detail || 'Action failed. Try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
      <div className="hero" style={{ borderRadius: 0, border: 'none', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 48 }}>
        ...
        <div className="mono text-muted" style={{ fontSize: 11, letterSpacing: 1.2 }}>BENGALURU · LIVE · 248 HOSTS</div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48, background: 'var(--bg)' }}>
        <div className="card shadow" style={{ maxWidth: 420, width: '100%' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: 18, fontSize: 13 }}>
            ...
            <span style={{ color: 'var(--cobalt)', cursor: 'pointer', fontWeight: 600 }} onClick={() => { setIsRegister(!isRegister); setErr(null); }}>
